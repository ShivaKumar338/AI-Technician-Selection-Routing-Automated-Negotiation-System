...
-- 3. WhatsApp negotiation sessions table
CREATE TABLE IF NOT EXISTS whatsapp_negotiations (
    ...
    status TEXT NOT NULL DEFAULT 'initiated'
        CHECK (status IN ('initiated','in_progress','agreed','rejected','timeout','error')),
...
CREATE TABLE IF NOT EXISTS whatsapp_messages (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    negotiation_id UUID NOT NULL REFERENCES whatsapp_negotiations(id) ON DELETE CASCADE,
    job_id UUID NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
